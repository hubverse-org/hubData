...
test_that("format_to_hub_model_output() expected output, default point_output_type", {
  ...
  zoltar_col_types <- c(
    ...
    NA,
    NA
  ) # based on "cDcccc????d????" from zoltr::job_data()
  zoltar_forecasts <- utils::read.csv(
    zoltar_forecast_csv_file,
    stringsAsFactors = FALSE,
    colClasses = zoltar_col_types
  ) # "NA" -> NA
  zoltar_forecasts["" == zoltar_forecasts] <- NA # "" -> NA
  zoltar_targets_df <- data.frame(
    name = c(
      "1 wk ahead inc case",
      "above baseline",
      "pct next week",
  ...
  expect_warning(
    act_data_frame <- format_to_hub_model_output(
      zoltar_forecasts,
      zoltar_targets_df,
  ...
  exp_data_frame <- utils::read.csv(
    "testdata/zoltar_data/hub_model_output_median.csv",
    stringsAsFactors = FALSE,
    colClasses = hub_col_types
  ) |> # "NA" -> NA
    dplyr::arrange(unit, target, output_type, output_type_id, value) |>
    tibble::tibble()
  expect_equal(act_data_frame, exp_data_frame)
...
test_that("format_to_hub_model_output() outputs warning for point conversions", {
  zoltar_point_forecast <- data.frame(
    model = "the-model",
    timezero = "2022-02-01",
    season = "2011-2012",
    unit = "US",
  ...
  zoltar_targets_df <- data.frame(
    name = "1 wk ahead inc case",
    numeric_horizon = 1
  )
  expect_warning(
    format_to_hub_model_output(
      zoltar_point_forecast,
      zoltar_targets_df,
      point_output_type = "median"
    ),
...
test_that("format_to_hub_model_output() expected output, mean point_output_type", {
  ...
  exp_data_frame <- utils::read.csv(
    "testdata/zoltar_data/hub_model_output_mean.csv",
    stringsAsFactors = FALSE,
    colClasses = hub_col_types
  ) |> # "NA" -> NA
    dplyr::arrange(unit, target, output_type, output_type_id, value) |>
...
test_that("missing model names throws error", {
  ...
  mockery::stub(
    validate_arguments,
    "zoltr::models",
    data.frame(model_abbr = c("the-model"))
  )
  expect_error(
    validate_arguments(
      NULL,
      "project 1",
      c("the-model", "bad-model"),
      NULL,
      NULL,
      NULL,
      NULL,
    ...
    regexp = "model not found in project",
    fixed = TRUE
  )
})
...
test_that("invalid timezero format throws an error", {
  ...
  expect_error(
    ...
    "one or more invalid timezero formats",
    fixed = TRUE
  )
})
...
test_that("missing timezero throws an error", {
  ...
  mockery::stub(
    validate_arguments,
    "zoltr::timezeros",
    data.frame(timezero_date = c("2022-02-01"))
  )
  expect_error(
    validate_arguments(
      NULL,
...
test_that("missing target throws an error", {
  ...
  mockery::stub(
    validate_arguments,
    "zoltr::targets",
    data.frame(name = c("1 wk ahead inc death"))
  )
  expect_error(
    validate_arguments(
      NULL,
