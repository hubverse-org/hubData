...
or the `hubData` wrapper `collect_hub()` which converts the output to a `model_out_tbl` class object where possible:
...
Or you could use `arrow::to_duckdb()` to first convert the dataset connection to an in memory virtual DuckDB table. This will allows you to run queries that are supported by DuckDB but not by arrow, extending the potential queries that can be run against hub data before collecting.
